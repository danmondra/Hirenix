...
import { searchByProfileGenerator } from '@/helpers/promptGenerators'
...
export async function POST(req) {
  ...
  const [systemPrompt, userPrompt, responseExample] = searchByProfileGenerator(userProfileGeneratedDecoded)
  ...
  const country = dataCleaned.country
    .toLowerCase()
    .replaceAll(' ', '-')
    .replaceAll('ñ', 'n')
    .str.normalize('NFD').replace(/\p{Diacritic}/gu, '')

  return NextResponse.json({ ...dataCleaned, country })
}
